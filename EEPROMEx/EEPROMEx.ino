...
void writeAndReadDouble() { 
    ...
    double output = 0.0;
    EEPROM.writeDouble(addressDouble,input);   //    EEPROM.writeDouble(addressDouble,input);   
...
void writeAndReadByteArray() {
    ...
    int writtenBytes = EEPROM.writeBlock<byte>(addressByteArray, input, itemsInArray);
    ...
    Serial.print("Total of written bytes by write: "); 
...
//    while (!EEPROM.isReady()) { delay(1); endMillis++; }
    ...
    // Time to write 7 byte array with 7 new values
    startMillis = millis();    
    EEPROM.writeBlock<byte>(addressByteArray, arraydif7, itemsInArray);
    endMillis = millis(); 
    Serial.print("Time to write 7 byte array with 7 new values (ms): ");    
    Serial.println(endMillis-startMillis); 

    // Time to write 7 byte array with 3 new values
    startMillis = millis();    
    EEPROM.writeBlock<byte>(addressByteArray, arrayDif3, itemsInArray);
    endMillis = millis(); 
    Serial.print("Time to write 7 byte array with 3 new values (ms): ");    
    Serial.println(endMillis-startMillis);
