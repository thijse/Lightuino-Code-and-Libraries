...
void TimeUtils::parseTime(const char* timeChar) {
	...
	if (hour > 24) {
		Usb.println("Day must be between 0 and 24");
		return;
	}
	...
	if (minute > 60) {
		Usb.println("Minute must be between 0 and 60");
		return;
	}
	...
	if (second > 60) {
		Usb.println("Second must be between 0 and 60");
		return;
	}
...
void TimeUtils::parseDate(const char* dateChar) {
	...
	if (year < 2000 || year >= 2100) {
		Usb.println("Year must be between 2000 and 2099");
		return;
	}
	...
	if (month < 1 || month > 12) {
		Usb.println("Month must be between 1 and 12");
		return;
	}
	...
	if (day < 1 || day > len) {
		Usb.print("Day must be between 1 and ");
		Usb.println(len, DEC);
		return;
	}
...
void TimeUtils::printDate() {   
	_timeElements = getTimeElements();
    Usb.print(days[weekday(_time) - 1]);
    Usb.print(_timeElements.Day, DEC);
    Usb.print(months[_timeElements.Month - 1]);
    Usb.print(tmYearToCalendar(_timeElements.Year), DEC);
}
