	 ...
	 _closeDev(closeDev)     
{
   ...
   if (_writer==NULL)  Usb.println("Error: No Memory writer function defined.");
   if (_reader==NULL)  Usb.println("Error: No Memory reader function defined.");
...
void MemIO::setMemPool(int base, int memSize) {
	...
	#ifdef _MEMIO_DEBUG  
	if (_nextAvailableaddress != _base) 
		Usb.println("Cannot change base, addresses have been issued");

	if (memSize < _nextAvailableaddress )  
		Usb.println("Cannot change ceiling, below issued addresses");
	#endif	
...
int MemIO::getAddress(int noOfBytes){
	...
	if (_nextAvailableaddress > _memSize) {
		Usb.println("Attempt to write outside of memory");
...
bool MemIO::isWriteOk(int address)
{
	...
	if (_allowedWrites != 0 && _writeCounts > _allowedWrites ) {
		Usb.println("Exceeded maximum number of writes");
		Usb.print("_writeCounts ");
		Usb.println(_writeCounts);
		Usb.print("_allowedWrites ");
		Usb.println(_allowedWrites);
